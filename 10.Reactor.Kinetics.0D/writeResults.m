function status = writeResults(time,y,flag)
  ...
  global g fuel gap ingas clad cool fileID iTimeStep kin
  ...
  switch flag
      ...
      otherwise
        if y(end) ~= y(end-1)
           ...
           fprintf(fileID,'s.pow(iTimeStep)=%12.5e; %%s\n',fuel.pow/fuel.pow0);
           fprintf(fileID,'s.reacDop(iTimeStep)=%9.2f; %%pcm\n',kin.reacDop*1e5);
           fprintf(fileID,'s.reacCool(iTimeStep)=%9.2f; %%pcm\n',kin.reacCool*1e5);
           fprintf(fileID,'s.reac(iTimeStep)=%9.2f; %%pcm\n',kin.reac*1e5);
           fprintf(fileID,'s.cDNP(:,iTimeStep)= ['); 
           fprintf(fileID,'%12.5e ',kin.cDNP); 
           fprintf(fileID,']; ');
           fprintf(fileID,'%% delayed neutron precursor density (1/m3)\n');
